...
$query = "SELECT my_Order.order_id, my_Order.customer_id, 
    my_Order.order_status, my_OrderItem.* FROM my_OrderItem, my_Order
...
function displayReceiptHeader()
{
    ...
    $time = date('g:ia');
    echo
    "<p class = 'w3-center'><strong>* * * * R E C E I P T * * * *</strong>
    </p><h4 class = 'w3-center'> Payment recieved from
    $_SESSION[salutation]
    $_SESSION[first_name]
...
function displayItemAndReturnTotalPrice($db, $row)
{
    ...
    "<tr>
        <td class = 'w3-center'>
            <img width='70' src='images/products/$imageFile' 
            alt='Product Image'>
        </td><td>
            $rowProd[name]
...
function displayReceiptFooter($grandTotal)
{
    ...
    <tr>
        <td colspan = '5'>
        <p class = 'w3-center'>Your order has been processed.<br>
        Thank you very much for shopping with Hamro Jersey Pasal.<br>
        We appreciate your purchase of the above product(s).<br>
        You may print a copy of this page for your permanent record.<br>
        To return to our e-store options page please
...
{
    $query = "UPDATE my_Order SET order_status = 'PD'
...
{
    $query = "SELECT * FROM my_OrderItem WHERE order_id='$orderID'";
    $orderItems = mysqli_query($db, $query);
    if ($orderItems != NULL) {
        $numRecords = mysqli_num_rows($orderItems);
    } else {
        echo
        "Error: SELECT failure in markOrderItemsPaid in checkoutxProcess.";
        exit(0);
    }
    for ($i = 1; $i <= $numRecords; $i++) {
        $row = mysqli_fetch_array($orderItems, MYSQLI_ASSOC);
        $query = "UPDATE my_OrderItem SET order_item_status = 'PD' WHERE 
        order_item_id = $row[order_item_id] and order_id = $row[order_id]";
        $success = mysqli_query($db, $query);
        if (!$success) {
...
function reduceInventory($db, $productID, $quantityPurchased)
{
    ...
    $query = "UPDATE my_Product SET quantity = $row[quantity]
