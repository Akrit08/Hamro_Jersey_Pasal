<!-- This page is form registration process-->
<?php
$salutation = $firstName = $middleInitial = "";
...
if (emailAlreadyExists($db, $_POST['email'])) {
    ...
    if ($uniqueLoginName != $_POST['loginName']) {
        echo "<h3>Your prefered login name already exists. So we have 
    ...
    (
        ...
        phone, street, city, region, postal_code, date_time, login_name, 
        login_password
    )
    VALUES
    (
    ...
    if (mysqli_query($db, $query)) {
        echo "<h3>Thank you for registering with Hamro Jersey Pasal.<br>
...
function getUniqueLoginName($db, $loginName)
{
    ...
    if ($numRecords != 0) {
        ...
        do {
            $count++;
            $uniqueLoginName = $loginName . $count;
            $query =
                "SELECT * FROM my_Customers
                WHERE login_name = '$uniqueLoginName'";
            $customers = mysqli_query($db, $query);
