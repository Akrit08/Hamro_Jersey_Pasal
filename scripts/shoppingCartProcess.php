<!-- Hanldes interaction between thw user and the database for Shopping Cart
Transactions.  -->
<?php
...
if ($numRecords == 0 && $productID == 'view') {
    ...
    if ($productID != 'view') {
        if ($retryingQuantity) {
            echo
            "<tr>
                <td class = 'w3-center' colspan ='7'>
                Please re-enter a non-zero product quantity not exceeding 
                the number in stock.
                </td>
            </tr>";
        }
...
function createOrder($db, $customerID)
{
    ...
    (
        customer_id, order_status, date_time
    ) VALUES
    (
       '$customerID', 'IP', SYSDATE() 
    )";
...
function displayHeader()
{
    ...
    <table class='w3-table w3-border w3-bordered'>
        <tr>
            ...
            <th class='w3-center'># in Stock</th>
